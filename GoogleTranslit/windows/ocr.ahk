...
ocr(file, lang := "FirstFromAvailableLanguages")
{
   ...
   if (file = "ShowAvailableLanguages")
   {
      ...
      loop % count
      {
         ...
         if (bool = 1)
         {
            DllCall(NumGet(NumGet(LanguageTest+0)+6*A_PtrSize), "ptr", LanguageTest, "ptr*", hText)  ; 6*A_PtrSize gives LanguageTag, 7 LocalizedName, 8 Autonym, 9 character set(?), 
            buffer := DllCall("Combase.dll\WindowsGetStringRawBuffer", "ptr", hText, "uint*", length, "ptr")
            id := StrGet(buffer, "UTF-16")
            DllCall(NumGet(NumGet(LanguageTest+0)+7*A_PtrSize), "ptr", LanguageTest, "ptr*", hText)  ; 6*A_PtrSize gives LanguageTag, 7 LocalizedName, 8 Autonym, 9 character set(?), 
            buffer := DllCall("Combase.dll\WindowsGetStringRawBuffer", "ptr", hText, "uint*", length, "ptr")
